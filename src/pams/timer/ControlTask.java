 ...
 *
 */
public class ControlTask extends CTimerTask {
	//作物信息管理器
	...
	@Override
	public void run() {
		System.out.println("controltask");
		i++;
		...
		for(Shed shed : sheds){
			System.out.println("shed_"+shed.getId());
...
//			}
	...
	private void warnsRecord(int deal,int level,String detail,int type,Shed shed){
		warn = warnManager.get(shed.getId(), type);
		boolean updateable;
		if(warn != null){
			updateable = true;
		}else{
