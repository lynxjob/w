...
Long.ShedGrid = Ext.extend(Long.BaseGeneralGrid, {
    ...
    submitAddForm : function(){
        ...
        if(form.isValid()==false){
         ...
         if(isNaN(parseInt(form.findField('crop_add').getValue()))){
            cropId = 0;
        }else{
            cropId = form.findField('crop_add').getValue();
        }
        form.submit({
            waitTitle : '请稍后',
            waitMsg : '正在提交信息···',
    ...
    submitEditForm : function(){
        ...
        if(isNaN(parseInt(form.findField('crop_edit').getValue()))){
            cropId = 0;
        }else{
            cropId = form.findField('crop_edit').getValue();
        }
        form.submit({
            waitTitle : '请稍后',
            waitMsg : '正在提交信息···',
