...
var wstatus = 4;								 //窗口状态(单个或多个,默认为多个即4个)
var curWindow = 1;							     //当前窗口
/*************************************************
  Function:     LogMessage
  Description:  写执行结果日志
  Input:        msg:日志
...
{  
    ...
    for(var i = 1; i <= 4; i ++)
    {
        if(i == iWindowNum)
        {
            ...
            if(wstatus == 1){
            	document.getElementById("NetPlayOCX" + i).style.display = "";
            	document.getElementById("NetPlayOCX" + i).style.height = iMaxHeight;
            	document.getElementById("NetPlayOCX" + i).style.width = iMaxWidth;
            }
        }
        else
        {
...
Long.VideoMonitorPanel = Ext.extend(Ext.Panel,{
   ...
   this.accordion1= new Ext.Panel({
      ...
      items:[
	        ...
	        {xtype:'button',text:'单个显示',name:'单个显示',style:'marginLeft:60px;marginTop:10px',handler:function(){ButtonPress('Window:one');}},
	        {xtype:'button',text:'多个显示',name:'多个显示',style:'marginTop:10px',handler:function(){ButtonPress('Window:more');}}
            ]
         },
         {
            xtype:'fieldset',
